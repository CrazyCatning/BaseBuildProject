...
class SaveIf {
 public:
  virtual ~SaveIf() {}
  virtual int32_t save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id) = 0;
};
...
class SaveNull : virtual public SaveIf {
 public:
  virtual ~SaveNull() {}
  int32_t save_data(const std::string& /* name */, const std::string& /* password */, const int32_t /* player1_id */, const int32_t /* player2_id */) override {
    int32_t _return = 0;
    return _return;
  }
};
...
typedef struct _Save_save_data_args__isset {
  _Save_save_data_args__isset() : name(false), password(false), player1_id(false), player2_id(false) {}
  bool name :1;
  bool password :1;
  bool player1_id :1;
  bool player2_id :1;
...
class SaveClient : virtual public SaveIf {
 ...
 public:
  ...
  int32_t save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id) override;
  void send_save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id);
...
class SaveMultiface : virtual public SaveIf {
 ...
 public:
  int32_t save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id) override {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->save_data(name, password, player1_id, player2_id);
    }
    return ifaces_[i]->save_data(name, password, player1_id, player2_id);
  }
  ...
  SaveConcurrentClient(std::shared_ptr< ::apache::thrift::protocol::TProtocol> iprot, std::shared_ptr< ::apache::thrift::protocol::TProtocol> oprot, std::shared_ptr< ::apache::thrift::async::TConcurrentClientSyncInfo> sync) : sync_(sync)
{
 ...
 public:
  ...
  int32_t save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id) override;
  int32_t send_save_data(const std::string& name, const std::string& password, const int32_t player1_id, const int32_t player2_id);
